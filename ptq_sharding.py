...
def main():
    ...
    del model
    quant_model.eval()
    model = paddle.fluid.dygraph.nn.Linear(1, 1)
    optimizer = paddle.optimizer.AdamW(learning_rate=0.001, parameters=model.parameters(), weight_decay=0.00001)
    type(quant_model)
    dir(quant_model)
    quant_model, optimizer, _ = group_sharded_parallel(quant_model, optimizer, "p_g_os")
    ...
    model_to_save = quant_model._layers if isinstance(quant_model, paddle.DataParallel) else quant_model
    ptq.save_quantized_model(model_to_save, 'dynamic_model/opt-2.7b/model', input_spec)
